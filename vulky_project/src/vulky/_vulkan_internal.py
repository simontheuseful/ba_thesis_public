...
class CommandPoolWrapper:
    ...
    def flush_frozen(self, manager, wait_for_completation=True):
        if _torch.cuda.is_available():
            _torch.cuda.synchronize(self.device.torch_device)
...
class DeviceWrapper:
    def __init__(self, device_index: int, enable_validation_layers):
        ...
        if __CUDA_IS_AVAILABLE__:
            self.torch_device = _torch.device('cuda:'+str(device_index))
        else:
        ...
        self.memory_manager = VulkanMemoryManager(self.vk_device, self.device_index, self.__physical_device)
