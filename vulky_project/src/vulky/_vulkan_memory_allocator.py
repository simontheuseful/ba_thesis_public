        ...
        with self.locker:
            ...
            for l in range(start_index, len(self._free_blocks)):
                ...
                if node is None:  # No block found
                    continue
                self._free_blocks[l].remove(node)
                b : _Block = node.value
...
class VulkanMemoryPage:
    ...
    def __init__(self, vk_device, device_index, capacity, memory_index: int, is_cpu: bool, is_gpu: bool):
        import os
        ...
        if is_gpu:
            if os.name == 'nt':
                ...
                is_valid = is_valid_fd(self.memory_vk_handle)
            if _torch.cuda.is_available():  # In a future to support other Non-nvidia general gpu implementations we need a switch here
                ...
                current_torch_device = _torch.cuda.current_device()
                _torch.cuda.set_device(self.device_index) # temporary set for properly import memory
                r, self.memory_cuda_mem = cudart.cudaImportExternalMemory(external_mem_hd)
                _torch.cuda.set_device(current_torch_device)
            ...
            if is_cpu:
                ...
                if self.support_cuda:
                    self.memory_as_tensor[t] = ViewTensor.from_blob(self.memory_cuda_ptr, (capacity // size,), t,
                                                                    _torch.device('cuda:'+str(self.device_index)), owner=self).as_subclass(_torch.Tensor)
    ...
    def destroy(self):
        if self.memory_as_tensor is None:
            return
...
class VulkanMemory:
    def __init__(self, page_allocator: VulkanMemoryPage, offset, size):
        ...
        if _torch.cuda.is_available() and self.page_allocator.is_gpu:
            self.torch_device = _torch.device('cuda:'+str(self.page_allocator.device_index))
        else:
            self.torch_device = _torch.device('cpu')
    ...
    def to_tensor(self, *shape: int, dtype: _torch.dtype) -> ViewTensor:
        return ViewTensor.from_blob(self.cuda_ptr, shape, dtype, self.torch_device, owner=self)
...
class VulkanAllocator:
    ...
    def __init__(self, vk_device, device_index, memory_index: int, memory_properties):
        self.pages = []
        self.vk_device = vk_device
        self.device_index = device_index
    ...
    def _add_page(self, size, alignment):
        ...
        page = VulkanMemoryPage(self.vk_device, self.device_index, page_capacity, self.memory_index, self.is_cpu, self.is_gpu)
...
class VulkanMemoryManager:
    ...
    def __init__(self, vk_device, device_index, vk_physical_device):
        import os
        self.vk_device = vk_device
        self.device_index = device_index
        ...
        if self.tensor_cpu_index not in self.allocators:
            self.allocators[self.tensor_cpu_index] = VulkanAllocator(self.vk_device, device_index, self.tensor_cpu_index, self.mem_properties.memoryTypes[self.tensor_cpu_index].propertyFlags)
        if self.tensor_gpu_index not in self.allocators:
            self.allocators[self.tensor_gpu_index] = VulkanAllocator(self.vk_device, device_index, self.tensor_gpu_index, self.mem_properties.memoryTypes[self.tensor_gpu_index].propertyFlags)
    ...
    def allocate_memory_for_tensor(self, size, memory_location):
        ...
        if index not in self.allocators:
            self.allocators[index] = VulkanAllocator(self.vk_device, self.device_index, index, self.mem_properties.memoryTypes[index].propertyFlags)
