...
class ObjectBufferAccessor:
    ...
    def __getattr__(self, item):
        ...
        if item not in self._rdv_fields:
...
class DeviceManager:
    ...
    def __bind__(self, w_device: _internal.DeviceWrapper):
        ...
        self.torch_device = w_device.torch_device
...
def syncronize_external_computation():
    global __ACTIVE_DEVICE__
    if __ACTIVE_DEVICE__ is not None and _torch.cuda.is_available():
        _torch.cuda.synchronize(__ACTIVE_DEVICE__.torch_device)


def torch_device():
    global __ACTIVE_DEVICE__
    if __ACTIVE_DEVICE__ is not None:
        return __ACTIVE_DEVICE__.torch_device
...
def sampler_linear(address_U: AddressMode = AddressMode.REPEAT,
    ...
    """
    return __ACTIVE_DEVICE__.create_sampler(
        Filter.LINEAR, Filter.LINEAR, MipMapMode.POINT, address_U, address_V, address_W, mip_LOD_bias, enable_anisotropy,
        max_anisotropy,
        enable_compare, compare_op, min_LOD, max_LOD, border_color, use_unnormalized_coordinates
    )
