...
float transmittance_rm_two_level_dda(MAP_DECL, vec3 x, vec3 w, float d, float step_size, float scale)
{
    ...
    vec3 ray_inv = vec3(1.0) / ray_dir; // how many t's are needed for one block per axis (not yet absolute value)

    ivec3 coord = clamp(ivec3(floor(ray_org)), ivec3(0), macro_dims - ivec3(1)); // macro block coordinate is easy because we are in macro block space
    ivec3 step = ivec3(sign(ray_inv)); // step direction for each axis
    vec3 delta = abs(ray_inv); // how many t's are needed for one block per axis

    vec3 select = vec3(0.5) + 0.5 * sign(ray_inv); // select = 1 if ray_inv > 0, else 0
    vec3 planes = vec3(coord) + select; // next possible planes to exit the macro block
    vec3 t_next = (planes - ray_org) * ray_inv; // for each axis the t value that gets to the next plane (macro block exit)

    // variables important for loop: t_next, coord, step, delta
...
FORWARD {
    ...
    _output[0] = transmittance_rm_two_level_dda(_this, x, wo, d, parameters.step_size, parameters.extinction_scale);
}

BACKWARD {
    NOT_SUPPORTED("Backward not implemented for transmittance_rm_two_level_dda_padded");
}
