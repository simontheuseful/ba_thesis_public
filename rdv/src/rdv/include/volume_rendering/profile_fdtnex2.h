...
FORWARD {
    ...
    _output[count_samples] = 1 - exp(-tau * 0.2); // 1 - 1 / (1 + tau*0.5); // 1 - exp(-tau);
    ...
    if (check_t != POSINF)
    {
        ...
        for (int i=0; i<SPECTRAL_DIM; i++)
            // nee contribution
            _output[count_samples + 1 + i] = env_sampler[3 + i] * rho_nee * nee_T; // env * phase * albedo * transmittance
        ...
        ray_box_intersection(sample_pos, light_dir_object_space, tMin, tMax);
        nee_T = transmittance_rt(_this, sample_pos, light_dir_object_space, tMax, 0.2);
        for (int i=0; i<SPECTRAL_DIM; i++)
            // nee contribution
            _output[count_samples + 1 + SPECTRAL_DIM + i] = env[i] * nee_T; // env * phase * albedo * transmittance
        for (int i=0; i<SPECTRAL_DIM; i++)
            _output[count_samples + 1 + 2 * SPECTRAL_DIM + i] = 1 / (1 - log(1-sa[i])); // albedo
        _output[count_samples + 1 + 3 * SPECTRAL_DIM] = g; // anisotropy
    }
}