...
FORWARD {
    ...
    if (check_t < POSINF) // any sample
    {
        ...
        if (tMax <= 0 || tMin > tMax) // should not happend but just in case
            return;
        // _output[count_samples + 1] = 1 / (1 - log(max(0.001, transmittance_rm(_this, sample_pos, light_dir_object_space, tMax, parameters.step_size))));
        _output[count_samples + 1] = transmittance_rm(_this, sample_pos, light_dir_object_space, tMax, parameters.step_size, 0.1);
    }
}