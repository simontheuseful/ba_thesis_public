...
//                break; // transmittance is too low, can stop early
    ...
    //                break; // transmittance is too low, can stop early
        ...
        if (current_depth_index > 0) // if we have at least one valid sample, we can compute nee and scattering for the first sample
        {
            ...
            float nee_tau = max(0.0, -log(nee_T + 0.001));
            for (int i=0; i<SPECTRAL_DIM; i++)
                // nee contribution
                _output[DEPTH_SAMPLES + 1 + i] = env_sampler[3 + i] * nee_T * rho_nee; // env * phase * albedo * transmittance
            ...
            for (int s=0; s <  env_samples; s++)
            {
                ...
                for (int i=0; i<SPECTRAL_DIM; i++)
                    // nee contribution
                    _output[DEPTH_SAMPLES + 1 + SPECTRAL_DIM + i] += nee_T * env[i] / env_samples; // env * phase * albedo * transmittance
            }
            for (int i=0; i<SPECTRAL_DIM; i++)
                _output[DEPTH_SAMPLES + 1 + 2 * SPECTRAL_DIM + i] = pow(sa[i], tau*(1-g));// 1 / (1 - log(1-sa[i])); // albedo
