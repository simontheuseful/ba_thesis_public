...
FORWARD {
    ...
    if (t != POSINF)
    {
        x += w * t;
        N = vec3(
            x.x <= -.99999 ? -1.0 : x.x >= .99999 ? 1.0 : 0,
            x.y <= -.99999 ? -1.0 : x.y >= .99999 ? 1.0 : 0,
            x.z <= -.99999 ? -1.0 : x.z >= .99999 ? 1.0 : 0
        );
        transform_normal_to_world(N, M);
    }
    // obtain normal from hit point in local space
    _output = float[](
        t,
        N.x, N.y, N.z,
        N.x, N.y, N.z,
        0.0, 0.0, // placeholder for UV
